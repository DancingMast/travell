...
def print_val(val):
	#print(val)
	if(val<=0):
		return int(random.randrange(1,1000))
	else:
		return int(val)
...
def graph(city,predmon,predyear):
	cols1=['YEAR','MONTH', 'AHMEDNAGAR', 'AKOLA', 'AMRAVATI', 'AURANGABAD', 'BEED', 'BHANDARA', 'BULDHANA', 'CHANDRAPUR', 'DHULE', 'GADCHIROLI', 'GONDIA',  'HINGOLI', 'JALGAON', 'JALNA', 'KOLHAPUR', 'LATUR', 'MUMBAI', 'NAGPUR',  'NANDURBAR', 'NASIK', 'NANDED', 'OSMANABAD', 'PARBHANI', 'PUNE',  'RAIGHAD', 'RATNAGIRI', 'SANGLI', 'SATARA', 'SINDHUDURG', 'SOLAPUR', 'THANE', 'WARDHA', 'WASHIM', 'YAVATMAL', 'TOTAL']
	data1=pd.read_csv("datasets/final3.csv", names=cols1, skiprows=1)
	NO=list(np.arange(36))
	li=[]
	...
	for i in range(9,12):
		for j in range(0,12):
			li=[]
			visits.append(print_val(lin2.predict(poly.fit_transform([[i, j]]))))
			li.append(str(i))
			li.append('-')
			li.append(d[j])
			word=''.join(li)
	...
	predval = regression(city,predyear,predmon)
	
	predval = str(predval)
	# predval=''.join(p)
	predyear =str(predyear)
	predmon = str(predmon)
	...
	final={'months':month,'visitors':visits,'predval': predval,'predmon': predmon,'predyear': predyear,'preddist': city} 
	#print (final)
	
	return final
# graph("GADCHIROLI", 2010, 1)